...
# 標記說明：
# [FLOW]    與轉換器/政策（model）相關的結構或運算
# [REWARD]  與獎勵計算相關的部分（以及使用獎勵做 PG 的地方）

# === [FLOW] Flow model ===
class RewardFlow(nn.Module):
    def __init__(self, dim=10, h=128, v=4, reward_fn=None, time_feat_dim=1, project_to_simplex=True):
        """
        dim: 序列長度
        h:   embedding/hidden 維度
        v:   vocab size
        reward_fn: 可微分的 reward(x_prob): x_prob [B,L,V] -> [B] 或 [B,L]
        time_feat_dim: 時間特徵維度（預設 1 = 直接丟標量 t）
        project_to_simplex: 是否在每步更新後用 softmax 投影回 simplex
        """
        super().__init__()
        self.dim = dim
        self.v = v
        ...
        self.project_to_simplex = project_to_simplex

        # 注意：第一層 input_dim = dim*h + time_feat_dim
        self.embed = nn.Embedding(v, h)
        self.net = nn.Sequential(
            nn.Linear(dim * h + time_feat_dim, h), nn.ELU(),
    ...
    def _time_features(self, t):
        # t 可能是 [B] 或 [B,1]；統一成 [B,1]
        if t.dim() == 1: 
            t = t[:, None]
        return t  # [B,1]

    # --- 支援 int token 與 soft prob 的 embedding ---
    def _token_features(self, x_t):
        """
        x_t: [B,L] (long) 或 [B,L,V] (float)
        return: [B, L, h]
        """
        if x_t.dim() == 2 and x_t.dtype in (torch.long, torch.int64):
            emb = self.embed(x_t)                         # [B,L,h]
        elif x_t.dim() == 3:
            # soft embedding: [B,L,V] @ [V,h] -> [B,L,h]
            emb = x_t @ self.embed.weight
        else:
            raise ValueError("x_t must be [B,L] long or [B,L,V] float")
        return emb

    def forward(self, x_t, t):
        """
        回傳向量場 v(x,t): [B, L, V]
        - 若在概率空間更新，建議令每個位置的向量場和為 0，較不易飄出 simplex。
        """
        B = x_t.shape[0]
        tfeat = self._time_features(t)                    # [B,1]
        tok_emb = self._token_features(x_t)               # [B,L,h]
        feat = torch.cat([tfeat, tok_emb.flatten(1, 2)], dim=-1)  # [B, 1 + L*h]
        v = self.net(feat).reshape(B, self.dim, self.v)   # [B,L,V]

        # 讓每個位置的向量場在 vocab 維度和為 0（保 simplex-friendly）
        v = v - v.mean(dim=-1, keepdim=True)
        return v

    def _project_simplex(self, x):
        # 最簡單投影：softmax。若你在 logit 空間更新，可換成別的投影。
        return F.softmax(x, dim=-1)

    def _flow_update(self, x, t, dt, return_process=False):
        """
        x: [B,L,V] 機率分佈（或 logits；此版假設是機率）
        t: [B,1] or [B]
        """
        v = self.forward(x, t)                  # [B,L,V]
        x_new = x + v * dt                      # Euler
        if self.project_to_simplex:
            x_new = self._project_simplex(x_new)

        last_x = x_new.argmax(dim=-1)           # [B,L]
        condt = v.detach()
        move_chance_t = torch.sigmoid(v.mean(dim=-1))  # [B,L]
        copy_flag = (move_chance_t > 0.5).float()      # [B,L]

        if return_process:
            return x_new, last_x, condt, move_chance_t, copy_flag
        return x_new

    def _flow_update_finetune(self, x, t, dt, copy_flag_temp=None, return_process=False, reward_weight=1.0):
        if self.reward_fn is None:
            raise RuntimeError("reward_fn is None. Please provide a differentiable reward_fn(x_prob).")

        x = x.detach().requires_grad_(True)
        v = self.forward(x, t)                              # [B,L,V]

        # 可微 reward（建議定義在機率上）：如 reward = x[..., 0].mean(dim=(1,1))
        reward = self.reward_fn(x)                          # [B] 或 [B,L]
        reward = reward.mean() if reward.dim() > 1 else reward.sum()
        reward_grad, = torch.autograd.grad(reward, x, retain_graph=False, create_graph=False)
        v_finetune = v + reward_weight * reward_grad        # [B,L,V]

        x_new = x + v_finetune * dt
        if self.project_to_simplex:
            x_new = self._project_simplex(x_new)
        x_new = x_new.detach()

        last_x = x_new.argmax(dim=-1)
        condt = v.detach()
        move_chance_t = torch.sigmoid(v.mean(dim=-1))
        copy_flag = copy_flag_temp if copy_flag_temp is not None else (move_chance_t > 0.5).float()

        if return_process:
            return x_new, last_x, condt, move_chance_t, copy_flag
        return x_new

    # --- 最小可用的 prior（均勻 Dirichlet） ---
    def _sample_prior(self, batch_size, length, device=None):
        device = device or next(self.parameters()).device
        # 均勻 Dirichlet -> [B,L,V] 機率
        alpha = torch.ones(batch_size, length, self.v, device=device)
        x0 = torch.distributions.Dirichlet(alpha).rsample()  # [B,L,V]
        return x0

    def _sample_finetune_flow(self, x, num_steps=64, eps=1e-5, batch_size=16, length=None,
                              copy_flag_temp=None, truncate_steps=0, reward_weight=0.3, device=None):
        """
        最小跑得起來的取樣器（不依賴 self.config/self.device 等外部物件）。
        """
        device = device or next(self.parameters()).device
        length = length or self.dim

        #x = self._sample_prior(batch_size, length, device=device)  # [B,L,V]
        timesteps = torch.linspace(1.0, eps, num_steps + 1, device=device)
        dt = (1.0 - eps) / num_steps

        last_x_list, condt_list, move_chance_t_list, copy_flag_list = [], [], [], []

        for i in range(num_steps):
            t = timesteps[i].expand(x.shape[0])  # [B]
            if i < num_steps - truncate_steps:
                x, last_x, condt, move_chance_t, copy_flag = self._flow_update(x, t, dt, return_process=True)
                x = x.detach()
                last_x = F.one_hot(last_x, num_classes=self.v).to(torch.float32).detach()
            else:
                x, last_x, condt, move_chance_t, copy_flag = self._flow_update_finetune(
                    x, t, dt, copy_flag_temp, return_process=True, reward_weight=reward_weight
                )
        ...
        # argmax trick（不丟 vocab 最後一維）
        x_argmax = x.argmax(dim=-1)                            # [B,L]
        x_argmax = F.one_hot(x_argmax, num_classes=self.v).to(torch.float32)

        x_out = x + (x_argmax - x).detach()
        return x_out, last_x_list, condt_list, move_chance_t_list, copy_flag_list
...
vocab_size = 4
batch_size = 256
model = RewardFlow(dim=seq_len, v=vocab_size)
optim = torch.optim.Adam(model.parameters(), lr=1e-3)

for step in range(2000):
    # 隨機初始序列（資料取樣；非必要標為 flow/reward）
    x0 = torch.randint(low=0, high=vocab_size, size=(batch_size, seq_len))
    t = torch.rand(batch_size)  # [FLOW] 時間條件輸入

    # [FLOW] 模型給出的轉換機率（政策分佈）
    logits = model(x0, t)   # [batch, seq_len, vocab_size]
    probs = F.softmax(logits, dim=-1)

    # [FLOW] 依政策分佈 Monte Carlo 抽樣候選 x1
    x1 = torch.distributions.Categorical(probs=probs).sample()

    # [REWARD] 計算 reward（偏好多少 A）
    r = reward_fn(x1, vocab_size)  # [batch]

    # [REWARD] 使用 reward 做 policy gradient 式 loss
    logp = F.log_softmax(logits, dim=-1)
    chosen_logp = logp.gather(-1, x1.unsqueeze(-1)).squeeze(-1)  # [batch, seq_len]
    loss = -(r.unsqueeze(1) * chosen_logp).mean()
    ...
    if step % 200 == 0:
        print(f"step {step}, loss={loss.item():.4f}, avg reward={r.mean().item():.3f}")

# === [FLOW] Sampling ===
x_t = torch.randint(low=0, high=vocab_size, size=(5, seq_len))
for t in torch.linspace(0, 1, 5):
    probs = F.softmax(model(x_t, torch.ones(len(x_t)) * t), dim=-1)  # [FLOW]
    x_t = torch.distributions.Categorical(probs=probs).sample()      # [FLOW]
    print(f"t={t:.1f}, samples: {x_t.tolist()}")
'''
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    config,
    eval=False,
    dim=10, h=128, v=4, reward_fn=None, time_feat_dim=1, project_to_simplex=True):
    super().__init__()
    ...
    self.dim = self.config.fm.dim
    self.v = self.config.fm.vocab_size
    self.h = self.config.fm.hidden_dim
    self.project_to_simplex = self.config.fm.project_to_simplex
  ...
  def _flow_update_finetune(self, x, t, dt, copy_flag_temp=None, return_process=False, reward_weight=1.0):
        ...
        if return_process:
            return x_new, last_x, condt, move_chance_t, copy_flag, v
        return x_new
  ...
  def on_save_checkpoint(self, checkpoint):
    ...
    if 'oracle_model' in self.__dict__:
        del self.__dict__['oracle_model']
    if 'oracle' in self.__dict__:
        del self.__dict__['oracle']
  ...
  def _compute_loss(self, batch, prefix):
    ...
    print(f"[debug] batch['seqs'].shape = {batch['seqs'].shape}")
  ...
  def training_step(self, batch, batch_idx):
    ...
    B = batch['seqs'].shape[0]
    x = F.one_hot(batch['seqs'], num_classes=4).float()  # [B, L, 4]
    x = x.permute(0, 2, 1)  # → [B, 4, L]

    # Lazy load oracle once, then reuse
    if not hasattr(self, 'oracle_model'):
        self.oracle_model = oracle.get_gosai_oracle("train").eval()
        self.oracle.eval()

    reward = self.oracle_model(x)
    reward = reward[:, 0].view(B)
    loss = reward - loss
    loss = loss.mean()
  ...
  def predict_reward_in_chunks(self, model, x, chunk_size=4):
    model.eval()
    ...
    with torch.no_grad():
        for i in range(0, len(x), chunk_size):
            ...
            if isinstance(out, torch.Tensor):
                ...
                raise TypeError(f"[chunk {i}] Unexpected model output type: {type(out)}")
    print(f"[chunk {i}] shape = {chunk.shape}")
    print(f"[chunk {i}] output shape = {out.shape if isinstance(out, torch.Tensor) else 'non-tensor'}")


    if len(outs) == 0:
        raise RuntimeError("No output collected from chunks")
    if len(set(o.shape[1:] for o in outs)) != 1:
        raise ValueError("Inconsistent shapes across chunks")
  ...
  def _forward_pass_diffusion(self, x0):
    ...
    if self.T > 0:
      diffusion_loss = self._d3pm_loss(xt=xt, x0=x0, t=t)
  ...
  def _d3pm_loss(self, x0, xt, t):
      """
      model_output: [B, L, K] logits
      x0:           [B, L]     原始 token 序列
      xt:           [B, L]     當前 noisy token 序列
      t:            [B]        時間步
      """
      model_output = self.get_logits_over_all_t(x0, self.config.T)
      T, B, L, K = model_output.shape
      q_theta = torch.softmax(model_output, dim=-1)  #Q^_s
      print("check")
      pred = torch.argmax(q_theta, dim=-1)
      pred = pred.view(T*B, L)  
      print("softmax finish")
      if self.initial_Q == None:
        self.initial_Q = q_theta[0]
        self.initial_Q = self.initial_Q.expand(T, -1, -1, -1) 

      term = - q_theta + self.initial_Q + q_theta * (torch.log(q_theta + 1e-8) - torch.log(self.initial_Q + 1e-8))
      
      q_theta = q_theta.transpose(2, 1)

      x = self.categorical_distribution(x0, self.config.T)
      x = x.view(T*B, L, K)
      x0_onehot = F.one_hot(pred, num_classes=K).to(self.dtype)
      x = x*x0_onehot
      x_bool = x.bool()
      x_bool = ~x_bool
      max, idx = torch.max(x, dim=-1, keepdim=True)
      x = x_bool.float()
      x = x*max
      x = x.view(T, B, L, K)
      punishment = x*term 
      ...
      return (alpha) * punishment


  def get_logits_over_all_t(self, x0, T):
      """
      對固定的一組時間 t = 1/T, 2/T, ..., 1 做完整 forward。
      回傳 logits_all: [T, B, L, K]
      """
      B, L = x0.shape
      device = x0.device

      # 建立均勻時間點：[1/T, 2/T, ..., 1]
      t_vals = torch.linspace(1 / T, 1.0, T, device=device)  # [T]
      logits_list = []

      for i, t_scalar in enumerate(t_vals):
        print(f"[step {i}] start | t_scalar = {t_scalar.item():.4f}")
        t = torch.full((B,), t_scalar, device=device)        # [B]
        sigma, _ = self.noise(t)                             # [B]
        unet_conditioning = sigma[:, None]                   # [B, 1]
        move_chance = 1 - torch.exp(-unet_conditioning)      # [B, 1]

        xt = self.q_xt(x0, move_chance)                      # [B, L]
        print(f"[step {i}] xt done")

        logits = self.forward(xt, unet_conditioning)         # [B, L, K]
        print(f"[step {i}] forward done | logits shape = {logits.shape}")
        logits_list.append(logits)

      logits_all = torch.stack(logits_list, dim=0)  # [T, B, L, K]
      return logits_all

  def categorical_distribution(self, x0, T):
      initial = self.initial_distribution(x0)
      B, L, K = initial.shape
      device = x0.device
      dt = self.config.sampling.dt
      temp = self.config.sampling.temp

      x_bar = initial # 初始 x₀ one-hot
      Q = initial     ####    #[B, L, K]
      standard = self.original_transformation()
      ...
      for t in range(T):
          if n ==0:
            ...
            pi = x_bar + dt * (Q * x_bar)  #[B,200, 4]

          if self.config.use_gumbel:
              g = self.sample_gumbel(pi.shape, ref_tensor=pi)
              pi = (pi + g) / temp

          x_bar = torch.softmax(pi, dim=-1)  # ← 下一個 x_t 來了！
          x_t_list.append(x_bar) 


      return torch.stack(x_t_list, dim=0)  # [T, B, L, K]

  def sample_gumbel(self, shape, ref_tensor):
    U = torch.rand(shape, device=ref_tensor.device, dtype=ref_tensor.dtype)
    return -torch.log(-torch.log(U + 1e-20) + 1e-20)

  def initial_distribution(self, x0):
      B, L = x0.shape
      mask_id = 4
      noise = torch.full((B, L), mask_id, device=x0.device)
      model_dtype = next(self.backbone.parameters()).dtype
      noise = F.one_hot(noise, num_classes=self.vocab_size).to(dtype=model_dtype)

      t = torch.zeros(B, device=x0.device)
      initial_distribution = self.backbone(noise, t)
      initial_distribution = initial_distribution.transpose(2, 1)
  ...
  def original_transformation(self):
    ...
    model_dtype = next(self.backbone.parameters()).dtype
    x = F.one_hot(x, num_classes=self.vocab_size).to(dtype=model_dtype)
    standard = self.backbone(x)
