...
from fm_dna import DiscreteFlowMatchingNet
import utils
import random
import string
import datetime
...
omegaconf.OmegaConf.register_new_resolver(
    "uuid",
    lambda: ''.join(random.choice(string.ascii_letters) for _ in range(10)) + '_' +
...
def patched_on_load_checkpoint(self, checkpoint):
    self.data_params = checkpoint.get("data_params", None)

L.on_load_checkpoint = patched_on_load_checkpoint


def _load_from_checkpoint(config):
    if 'hf' in config.backbone:
        return diffusion_gosai.Diffusion(config).to('cuda')
...
def _print_config(
    ...
    if save_cfg:
        with fsspec.open(
            f"{config.checkpointing.save_dir}/config_tree.txt", 'w'
        ) as fp:
...
def _print_batch(train_ds, valid_ds, test_ds):
    for dl_type, dl in [('train', train_ds), ('valid', valid_ds), ('test', test_ds)]:
        print(f'Printing {dl_type} dataloader batch.')
        batch = next(iter(dl))
        print('Batch seqs.shape', batch["seqs"].shape)
        print('tokens:', dataloader_gosai.dna_detokenize(batch["seqs"][0]))
...
def _train(config, logger):
    ...
    arch = getattr(config, "architecture", "diffusion")
    if arch == "flow":
        model = DiscreteFlowMatchingNet(config)
    else:
        model = diffusion_gosai.Diffusion(config)
...
@hydra.main(version_base=None, config_path='configs_gosai', config_name='config_gosai')
def main(config):
